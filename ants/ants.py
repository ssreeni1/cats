...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 7
        if isinstance(self.place.ant, NinjaAnt):
            return False
        else:
    ...
    def action(self, colony):
        ...
        # BEGIN EC
        if self.is_scared and not self.frightened:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while curr_place is not beehive:
            if curr_place.bees != [] and self.min_range <= range_counter and range_counter <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        for bee in self.place.bees[:]:
            ...
            if amount >= self.armor and bee.place:
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digesting > 0 or bee is None:
            self.digesting -= 1
        else:
            self.eat_bee(bee)
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 7
        for bee_samp in self.place.bees[:]:
            bee_samp.reduce_armor(self.damage)
        # END Problem 7
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        ...
        if self.contained_ant is not None:
            self.contained_ant.action(colony)
        for bee_invader in self.place.bees[:]:
            bee_invader.reduce_armor(self.damage)
    ...
    def action(self, colony):
        ...
        if self.imposter:
           self.reduce_armor(self.armor)
            ...
            curr_exit = self.place.exit
            while curr_exit is not None:
                if curr_exit.ant is not None:
                    if curr_exit.ant.is_container:
                        contained_one = curr_exit.ant.contained_ant 
                        if contained_one not in self.doubled_ants and contained_one is not None:
                            contained_one.damage *= 2
                            self.doubled_ants.append(contained_one)
                    if curr_exit.ant not in self.doubled_ants:
                        curr_exit.ant.damage *= 2 
                        self.doubled_ants.append(curr_exit.ant)
                curr_exit = curr_exit.exit
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    orig_act = bee.action
    new_act = effect(bee.action, bee)
    def new_effect(colony):
        nonlocal duration
        if duration > 0:
            new_act(colony)
        else:
            if effect.__name__ is "make_scare":
                bee.frightened = True
            orig_act(colony)
        duration -= 1
    bee.action = new_effect
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC
        if target and not target.frightened:
